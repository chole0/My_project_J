...
class CellPeople extends JLabel implements ListCellRenderer {
	CellPeople() {
		setOpaque(true);
	}
...
class ChatListModel extends AbstractListModel {

	private Vector vs;

	public ChatListModel(Vector vs) {
		this.vs = vs;
	}
...
public class ChatThreadRoom extends JFrame {
	...
	public ChatThreadRoom(String u_name, Socket client) {
		// 赋值
		name = u_name;
		clientSocket = client;
		onlines = new Vector();
		...
		contentPane = new JPanel() {
			...
			protected void paintComponent(Graphics g) {
				...
				g.drawImage(new ImageIcon("C:\\Users\\ASUS\\IdeaProjects\\My_project_J\\src\\Java_chatwork\\images\\ChatBackground.jpg").getImage(), 0, 0,
		...
		// 在线客户列表
		listmodel = new ChatListModel(onlines);
		list = new JList(listmodel);
		list.setCellRenderer(new CellPeople());
		...
		try {
			...
			// 消息提示声音
			file = new File("C:\\Users\\ASUS\\IdeaProjects\\My_project_J\\src\\Java_chatwork\\sound\\msg.wav");
			cb = file.toURL();
			aau = Applet.newAudioClip(cb);
			// 上线提示声音
			file2 = new File("C:\\Users\\ASUS\\IdeaProjects\\My_project_J\\src\\Java_chatwork\\sound\\Global.wav");
			cb2 = file2.toURL();
